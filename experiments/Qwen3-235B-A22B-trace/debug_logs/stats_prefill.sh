...
grep "get_num_new_matched_tokens -> request_finished" "$LOG_FILE" | \
awk '{print $(NF-1)}' | \
sort -n | \
...
END {
    if (NR > 0) {
        ...
        # For median, handle even/odd number of values
        if (p50_index == count * 0.5) {
        ...
        # Output as a single CSV row
        printf "%s,%.3f,%.3f,%.3f,%.3f,%.3f\n", filename, mean, median,p90, p99, max
    } else {
        # Output an empty row with filename if no data is found
        printf "%s,0,0,0,0,0\n", filename
    }
}'
